...
def BanfieldEmailimport():
    ...
    for m_id in messages['messages']:
        ...
        if dates == today:
            ...
            dataframe['Current_Status_Start_Date'] = pd.to_datetime(dataframe['Current_Status_Start_Date'],
                                                                    errors='coerce').dt.date
            dataframe['Posting_Date'] = pd.to_datetime(dataframe['Posting_Date'], errors='coerce').dt.date

            dataframe.dropna(how="all", inplace=True)

            dataframe2019 = pd.read_csv("gs://hc_banfield_ats/File_2019/Banfield-Historical-ATS-Data.csv")

            merged_df = dataframe2019.append(dataframe, True)

            merged_df['Current_Status_Start_Date'] = ""

            merged_df['Posting_Date'] = ""

            merged_df.drop_duplicates(keep='last', subset=[
                'Candidate_Identifier', 'Submission_Created_Date', 'Submission_Source', 'Submission_Source_Identifier',
            ...
            dataframe.to_csv(
                'gs://hc_banfield_ats/File_Completed/' + "Banfield_ATS_DATA " + str(today.month) + "." + str(today.day) +
